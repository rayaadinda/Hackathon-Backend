...
export const updateUserProfile = async (req, res) => {
  const userId = req.user.id;
  const { 
    name, phone, gender, date_of_birth, address, postal_code, country,
    nationalities, languages, enneagram, groups, volunteer_opportunities,
    preferred_duration, years_experience, longest_experience, availability,
    status_volunteer
  } = req.body;

  if (status_volunteer && !['open', 'closed'].includes(status_volunteer)) {
    return res.status(400).json({ error: 'Status volunteer must be either "open" or "closed"' });
  }
  ...
  try {
    const { data, error } = await supabase
      ...
      .update({ 
        ...
        volunteer_opportunities: typeof volunteer_opportunities === 'object' ? volunteer_opportunities : undefined,
        preferred_duration,
        years_experience,
        longest_experience,
        availability,
...
export const submitVolunteerForm = async (req, res) => {
  try {
    ...
    const nationalitiesArray = nationalities ? nationalities.split(',').map(item => item.trim()) : [];
...
async function findMatchingProjects(userId) {
  try {
    ...
    for (const project of projects) {
      ...
      if (project.project_type === 'study_group' && profile.group_study_groups) {
        matchScore += 25;
      ...
      if (project.required_skills && project.required_skills.length > 0) {
        ...
        if (profile.skill_financial && project.required_skills.includes('financial')) {
          matchScore += 15;
      ...
      if (project.min_experience && profile.years_experience) {
        if (
          ...
          (project.min_experience === '6 months - 1 year' && 
           ['6 months - 1 year', '> 1 year'].includes(profile.years_experience)) ||
          (project.min_experience === '> 1 year' && profile.years_experience === '> 1 year')
        ) {
          matchScore += 15;
