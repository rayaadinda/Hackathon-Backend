...
export const updateUserRole = async (req, res) => {
	...
	if (role !== "admin" && role !== "volunteer") {
		return res
			.status(400)
			.json({ error: 'Role must be either "admin" or "volunteer"' })
	}
...
export const updateProjectStatus = async (req, res) => {
	...
	if (!status_project || !["on_going", "done"].includes(status_project)) {
		return res
			.status(400)
			.json({ error: 'Status must be either "on_going" or "done"' })
	}
...
export const updateApplicationStatus = async (req, res) => {
	...
	if (!status || !["confirmed", "rejected"].includes(status)) {
		return res
			.status(400)
			.json({ error: 'Status harus "confirmed" atau "rejected"' })
	}
	...
	try {
		...
		if (application.status === status) {
			return res
				.status(400)
				.json({ error: `Aplikasi sudah dalam status "${status}"` })
		}
		...
		for (const volunteer of volunteers) {
			...
			if (
				...
				project.required_skills.forEach((requiredSkill) => {
					...
					if (
						...
						matchScore += 20
						matchReasons.push(
							`Memiliki keahlian yang dibutuhkan: ${requiredSkill}`
						)
					}
				})
			}
				...
				project.required_languages.forEach((requiredLanguage) => {
					if (
						volunteer.languages.some(
							(lang) => lang.toLowerCase() === requiredLanguage.toLowerCase()
						)
					) {
						matchScore += 20
						matchReasons.push(
							`Menguasai bahasa yang dibutuhkan: ${requiredLanguage}`
						)
					}
				})
			}
			...
			if (project.min_experience && volunteer.years_experience) {
				...
				if (volunteerExp >= projectMinExp) {
					matchScore += 20
					matchReasons.push(
						`Memiliki pengalaman yang cukup: ${volunteer.years_experience}`
					)
				}
			}
			...
			if (project.duration && volunteer.preferred_duration) {
				...
				if (projectDuration <= volunteerPref) {
					matchScore += 20
					matchReasons.push(
						`Durasi proyek sesuai preferensi: ${volunteer.preferred_duration}`
					)
				}
			}
...
export const assignVolunteers = async (req, res) => {
	...
	try {
		...
		if (project.status_project === "done") {
			return res
				.status(400)
				.json({ error: "Cannot assign volunteers to a completed project" })
		}
		...
		const closedVolunteers = volunteers.filter(
			(v) => v.status_volunteer === "closed"
		)
		...
		await supabase
			.from("projects")
			.update({
				current_volunteers: project.current_volunteers + assignments.length,
			})
			.eq("id", projectId)
...
// AI Matching API Proxy Controllers
const AI_API_BASE =
	"https://volunteer-project-matching-api-production.up.railway.app"

export const matchVolunteersAI = async (req, res) => {
	try {
		const response = await axios.post(`${AI_API_BASE}/match`, req.body)
...
export const batchMatchVolunteersAI = async (req, res) => {
	try {
		const response = await axios.post(`${AI_API_BASE}/match/batch`, req.body)
		return res.status(response.status).json(response.data)
	} catch (error) {
		const status = error.response?.status || 500
		const data = error.response?.data || { error: error.message }
...
export const validateAIMatchData = async (req, res) => {
	try {
		const response = await axios.post(`${AI_API_BASE}/validate`, req.body)
		return res.status(response.status).json(response.data)
	} catch (error) {
		const status = error.response?.status || 500
		const data = error.response?.data || { error: error.message }
